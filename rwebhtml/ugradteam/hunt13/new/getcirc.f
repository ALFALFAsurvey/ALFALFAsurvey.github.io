...
C
      ...
      CHARACTER TELCODE*1,SGN*1,TFNOTE*1
     ...
     *  XRADEG,RADEG
      ...
      open(15,file='junk.radec',status='unknown') 
      nsamp=0
      ...
      read(*,'(2i2,i3,a1,3i2)') irh,irm,irs10,sgn,idd,idm,ids

      xra = real(irh) + real(irm)/60. + real(irs10)/36000.
      if(xra.lt.1.0e-6) xra=1.0e-6
      xdec= real(idd)+ real(idm)/60.+ real(ids)/3600.
      if(sgn.eq.'-') xdec=-xdec
      xradeg=xra*15.
      xrar=xradeg*onedeg
      xdecr=xdec*onedeg

      write(*,'('' Enter search radius in degrees: '')')
      read *,searchradius
  
      write(*,'('' Enter min and max velocities : '')')
      write(*,'('' velmin=-999 to include no redshifts : '')')
      read *,velmin,velmax
   
      mindec=nint(3.*searchradius) 
      minra=nint(3.*searchradius/15.) 

      open(10,file='agc2000.north',
     *   status='old')

      if(sgn.ne.'-' .and.idd.ge.5) then
        open(10,file='agc2000.north',
     *   status='old')
         write(*,'('' Opening agc.north '')')
         go to 75
      endif
      if(sgn.eq.'-' .and.idd.ge.5)  then
        open(10,file='agc2000.south',
     *   status='old')
         write(*,'('' Opening agc.south '')')
         go to 75
      endif
      if(idd .le. abs(mindec+1) .and. idd.lt.5) then
       open(10,file='agc2000.bridge',
     *   status='old')
        write(*,'('' Opening agc.bridge '')')
        go to 75
      endif
     ...
     *   status='old')
      ...
      write(*,'('' No input file selected, terminating '')')
      go to 98
   75 write(*,'('' Opened file correctly'')')
c
...
c
      if(rah.gt.(irh+1+minra)) go to 99
      if(rah.lt.(irh-1-minra)) go to 1
      if(abs(decd-idd).gt.mindec) go to 1
  ...
  101 FORMAT(I6,A1,A8,1x,2I2.2,I3.3,A1,3I2.2,I5,2I4,i6,2x,I5,I3,2x,
     *       I5,I4,I5,I4,1x,A4,A1,i1,I2,2i3,i5,2i3,a1,f6.2)
      write(15,'(i6,2f12.5,f7.0)') AGCNUMBER,ra,dec,vel
